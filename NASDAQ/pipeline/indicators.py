...
LAST_DATE = os.environ.get("IND_LAST_DATE", "2022-12-31")  # stage 1: in-sample boundary

TRADING_DAYS = 252  # US equities, Mon-Fri
...
def main() -> int:
    ...
    for code, meta in manifest.items():
        ...
        df = enrich(df)
        if code != "IXIC":
            df["rs_bench_3m"] = df["ret_3m"] - tasi_ret63.reindex(df.index)
